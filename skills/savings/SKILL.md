...
4. **Write** nothing of your own above the fence, not the report's `exo cost report` title and not the opening line an output style asks for: the report is the whole answer, and a line above it repeats what the reader is about to read.
5. **Add** no prose of your own under the fence restating the report's rows, naming them an estimate, explaining the cost basis, converting refused text into tokens or money, or working out a saving or a net, because the report already says what it measures and that the saving is not.
...
- Measured: each guard's refusals with the bytes of file text they refused, and the tokens, price and wall time of the calls that were exo's own work, split into skill loads and re-reads after a refusal, plus exo's hook time. Not measured: helpers, the ladder and the instructions the session hook adds, and the report marks each. There is no saved or net figure, because the refused text was never sent, so it has no token count or price, and how often it would have been resent is an assumed baseline.
