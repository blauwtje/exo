---
name: explain-code
description: "Use when asked how X works or why it is so, to explain, teach or confirm it, if a stated reason holds even when a comment gives it, for a PR verdict, runbook, ADR or doc. Not for locations (exo:locate-code), libraries (check-docs), unproven failures (find-cause) or restructuring (audit-architecture)."
...
- Not for locations only: the `exo:locate-code` agent returns those.
- Not for how an external library or API behaves: `check-docs` owns it.
- Not for behavior reported wrong with an unproven cause: `find-cause` owns it.
- Not for where the architecture should change: `audit-architecture` owns it.
...
Discovery that spans many files goes to the `exo:locate-code` agent for locations; the ranges it returns are then read here, because a location is not an explanation.
...
| "A list of files answers how." | Locations are `exo:locate-code`'s job; `how` asks for the chain in prose. |
