---
name: design-ui
description: "Use when a page, component, or visual axis changes (type, color, spacing, motion, copy), including a redesign or a surface called empty, boring, or generic. When a new visual surface does not name its displayed data, settings, or behavior, define-scope decides those first; design-ui follows for presentation. Not for typo-only fixes or behavior with no visual effect."
...
This skill owns a visual change at any file count, since Build dispatches its own builders. An undecided surface (its displayed data, settings or behavior) goes to `define-scope` first; a change that also adds state, persistence, a dependency or a network call belongs to `build-change`, which borrows this skill for the look.
