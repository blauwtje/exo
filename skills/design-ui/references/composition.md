...
Name things by what the person controls and recognizes, never by how the system is built: someone manages *notifications*, not *webhook configuration*. Describe what a thing does instead of selling it — "Search across all 40 fields" beats "Powerful search", "Exports to CSV in one click" beats "Seamless integrations". Use established brand vocabulary when it exists; otherwise choose one register for the flow and keep its verbs, contractions, and formality consistent, since "All clear. Nothing needs you." and "0 open incidents" belong to different flows.

For every marketing claim, include at least one mechanism, constraint, example, or proof nearby, and never fill sections by paraphrasing one benefit. For app work, populate every named field with a normal value, each documented boundary, and a value that triggers each represented error state. Source values from the repository or subject materials; label invented values as assumptions, and never invent regulated, safety, customer, or performance claims.
