...
The text `implementing` hands a `general-purpose` delegate on `sonnet` when the branch review returns `FINDINGS`. The delegate repairs from the review report alone and leaves the Final verification to the session.
