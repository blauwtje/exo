...
// run-plan writes `<git-dir>/exo/run-plan.active` at its step 1, the plan path
// on its first line and the run's checkout on an optional second, and removes
// it at step 7; only a plan named there with an open task counts as running.
//
...
// The first ready task of the running plan, or null when no marker names a
// plan or every task landed.
export function runningPlan(cwd) {
  ...
  const [planPath, checkout] = fs.readFileSync(marker, 'utf8').split('\n').map((line) => line.trim());
