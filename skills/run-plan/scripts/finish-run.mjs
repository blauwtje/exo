...
// `origin/<default>` the way ship.mjs reads the default branch, runs
// pick-reviewer.mjs on that base exactly as run-plan's SKILL.md does, and
// removes step 1's `run-plan.active` marker so the hooks stop pushing a
// finished run.
//
//   node finish-run.mjs [--root <checkout>] [--reviewer <name>]
//
// Prints the reviewer pick-reviewer.mjs picked, or exits 1 with one line
// when the default branch or the merge-base cannot be read.
...
/** The reviewer line for `root`'s branch review, with step 1's marker removed once it is picked. */
...
function main(argv) {
  const flags = parseFlags(argv, { root: 'value', reviewer: 'value' });
  process.stdout.write(`${finishRun(flags.root ?? process.cwd(), { reviewer: flags.reviewer })}\n`);
}
