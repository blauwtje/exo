...
1. **Resolve the workspace, then start the run.** Settle where the run commits as `references/workspace.md` says. Run `node "${CLAUDE_SKILL_DIR}/scripts/start-run.mjs" --checkout <run's checkout>`, adding `--plan <path>` for a named plan. Name the plan path it prints; its marker survives a clear. A failure line on zero or several matching plans becomes the question which to run. Invoking run-plan authorizes commits in that workspace and a wave's worktrees beside it; a push or pull request waits for the user's answer to `ship`.
