...
5. **Dispatch the unit.** Each block goes to the `exo:run-unit` agent, naming the plan path, the branch, the checkout, `${CLAUDE_SKILL_DIR}` resolved as `<skill>`, and the block's task numbers; it dispatches every build, repair and wave for its tasks and lands them, so this session never builds, lands or repairs a block task itself: that work would fill it. Its return holds one `LANDED`, `OPEN` or `BLOCKED` line per task, or one `BUDGET:` line.
6. **Route the return.** `LANDED` needs nothing. `OPEN` or a `BUDGET:` line goes back to step 3 and a fresh `exo:run-unit` agent for the open tasks, never a finish here. `BLOCKED all nested dispatch unavailable` ends the turn: set `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` to 2 or more, with no fallback, because building here is the bloat the unit exists to avoid. `BLOCKED` with a question runs `node "${CLAUDE_SKILL_DIR}/scripts/resume-plan.mjs" wait`, so the stop does not block, asks it, then a fresh unit takes the rest from step 3; a second `OPEN` for one task with no new cause ends the turn with its report paths. Loop to step 3 silently; only a blocked task, a failed check or a user question earns a message.
