...
With `--run`, the same turn continues into `run-plan` on the written plan, a phased plan's first phase file, instead of asking. Otherwise a deliverable plan ends the turn on `node "${CLAUDE_SKILL_DIR}/../route-skills/scripts/next-stage.mjs" --after draft-plan --artifact <plan path>`'s output: the reply names the plan path and the task count, never the plan text, then that output.
