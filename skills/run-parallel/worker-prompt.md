...
Work only inside your slice; another worker owns every other one. Cd into your checkout before every command, because you start in the lead's working directory, not yours. Before your first edit, run `git rev-parse --show-toplevel` from inside your checkout and check it prints exactly the checkout path above; a different path stops you, so report verdict=BLOCKED with what it printed and what you expected. Give Edit and Write only absolute paths inside your checkout. You make no git write outside your checkout and ask the user no question; when a delete would get you past a blocked state, report BLOCKED with two or three options instead. How to verify: <the command or reading that proves the slice>.
...
Build the whole artifact in your checkout and nowhere else; other candidates build the same thing in theirs. Cd into your checkout before every command, because you start in the lead's working directory, not yours. When your checkout is a worktree, run `git rev-parse --show-toplevel` from inside it before your first edit and check it prints exactly the checkout path above; a different path stops you, so name what it printed and what you expected in your rationale and return status=blocked. Give Edit and Write only absolute paths inside your checkout. You ask the user no question; when a delete would get you past a blocked state, stop and name it in your rationale instead.
