...
4. **Fan out.** Send all N in one message, in the background, each a `general-purpose` delegate on `sonnet` from `worker-prompt.md`. Every worker writes its full detail to its file and returns a short verdict line; write an isolated worker's returned detail to `<run dir>/worker-<n>.md` and read that as its file. A dropout leaves N-1, noted in the report.
