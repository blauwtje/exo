...
1. **Frame.** Before any spawn, write `<git dir>/exo/run-parallel/<slug>/frame.md`: the done predicate, the shape, N, and each worker's slice, arm or check with its own output path. A rule written after results exist bends to them.
2. **Fix the judge.** Also in `<git dir>/exo/run-parallel/<slug>/frame.md`: a race names `first pass`, `rank all` or `best-of`; a gauntlet passes only when every check passes; a contest gets 3 to 6 gradeable criteria for what success means for this artifact. The rubric never enters a candidate brief, because a candidate that sees it writes to the grader.
...
5. **Aggregate.** A result missing its named SHAs or method reruns once; a second miss is a gap, and a gap never counts as a pass. Coverage needs a result for every slice; race and gauntlet apply the rule from `<git dir>/exo/run-parallel/<slug>/frame.md`. Coverage, race and gauntlet end here at step 8.
...
- A rule in `<git dir>/exo/run-parallel/<slug>/frame.md` outranks what the results suggest: a rule that looks wrong afterwards means a reframe and a rerun, never a quiet swap.
