...
2. **Order.** Draw the dependency edges between edits: `A → B` means B cannot land green before A. Two or more edges load `draft-plan` at inline depth before editing, unless an upstream `define-scope` brief already orders every edge. Settle a shared data shape's types before ordering behavior edits against it.
