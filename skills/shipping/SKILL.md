...
1. **Write the pull request body**, for `open-pr` and `pr-merge`, after reading `references/pr-prep.md`.
   It follows `## The pull request` in `../issuing/references/fields.md`: the goal, the proof line, and `Closes #<n>` for work from issue `<n>`.
   With no issue behind it, run `node "${CLAUDE_SKILL_DIR}/../issuing/scripts/repo-fields.mjs"` and its `--size` form for the fields.
...
| File | Read it when |
|---|---|
| `../issuing/references/fields.md` | Route step 1, for the body's fields. |
