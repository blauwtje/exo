...
Reading the `with`/`without` lines yourself grades your own change, and a close
...
- Step 4's `with`/`without` read is a close call: both lines could pass for
  the same reason, or the `with` line only reads more thorough.
- Not for a brand-new case's first pass: step 4 already proves that one, because
  the `without` line failing and the `with` line passing needs no judge.
...
2. Run `pressure.mjs --prompt <the same case file> --cells <cell> --plugin-dir
   <clone>` once per clone; each run's `without` line is the same skill-less
   answer twice over, so read only the `with` line from each.
3. Label the two `with` lines A and B, alternating which letter holds the
   changed answer so no pattern forms across repeated evals, and write only
   the label and the text to a scratch file the judge will see; keep the
   mapping yourself and never write "baseline", "changed", "old" or "new"
   into that file.
...
- The judge's blind pick outranks your own read of the `with`/`without`
  lines, because a close call is exactly what your own read cannot referee.
