...
2. **The steps never vary and a slip damages state.** A script the skill runs, named with its command. Example: the repo map command in `planning`.
3. **The content varies and the layout does not.** A template in a fenced block, every part named in order. Example: the question layout in `shaping`.
