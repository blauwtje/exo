...
The text `deepen` hands a `general-purpose` delegate on `opus` to read the scoped code and write the ranked cards, so the session itself reads no scoped code.
