...
A skill reads this file before a message of its asks the user to pick; the core of the shape also stays in the using-exo body, which every session holds.
...
3. **The recommended option is number 1**, in every question and in the next stage's fixed order; stopping or keeping things as they are comes last unless it is the recommended one. Label, marker and clause are in the reply's language, as the language rule under `# Context` in using-exo sets.
...
- One clear question outranks a complete one: a second question waits for the next turn, except in a `shaping` round.
