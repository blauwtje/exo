...
- `issues` creates one GitHub issue whose body opens with the line `<!-- exo:spec -->` followed by the brief in the Spec shape of the file-issues skill's fields file, with its sections, fields and relations as that file says and none of them decided here, and writes no file.
