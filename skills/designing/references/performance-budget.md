...
The 2.5s splits roughly TTFB 40%, resource load delay under 10%, load duration 40%, render delay under 10% ([web.dev/articles/optimize-lcp](https://web.dev/articles/optimize-lcp), updated 2025-03-31; the page calls these guidelines, not strict rules). A hero raster therefore gets about **one second** of loading on the target connection, and a ground built from CSS, SVG, or a gradient costs approximately nothing. That is a design fact before it is an engineering one: the ladder in the `implementation` reference from repository asset to subject artifact to real data to typographic treatment usually descends in cost as it ascends in specificity.
...
- Loading states reserve the space their result will occupy (the rule the `interaction-qa` reference already states, here with its measurement).
...
- Subset by `unicode-range` to the scripts the audience reads, and keep the language coverage the `typography` reference requires.
...
- **CSS scroll-driven animation runs off the main thread** ([developer.chrome.com/docs/css-ui/scroll-driven-animations](https://developer.chrome.com/docs/css-ui/scroll-driven-animations), updated 2023-05-05; Chrome/Edge 115+, Safari 26+). A scroll effect built on a JS `scroll` handler is the expensive form of the same idea, and the `implementation` reference already bans it.
...
- Content, accessibility, and the states in the `interaction-qa` reference outrank every budget: the fix for a slow page is never a removed state or an unlabelled control.
