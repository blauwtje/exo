...
The text `designing` hands a `general-purpose` delegate on `sonnet` for one direction comp, one dispatch per variant in one message. The session fills `RUN`, `INDEX`, `OUT` and `FRAME`.
...
Run no shell command, browser or build: the picker is the first render.
```
