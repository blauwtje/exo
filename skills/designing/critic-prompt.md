...
The text `designing` hands a `general-purpose` delegate on `opus` for the post-build round; the final round resumes the same delegate. The session fills `RUN`, `URL`, `ROUND` and `SKILL`.
...
Capture with `--label final` into `$RUN/renders`, rerun check-ui at both viewports into `check-ui-final-390.json` and `check-ui-final-1440.json`, open the final renders once, and write `$RUN/verdict.md`: one line per fault from `$RUN/faults.md` (`resolved`, `partial`, or `unresolved`, tied to what the final render shows), at most three regressions the repairs introduced, and a closing `disposition:` line recomputed from what remains open. A partial or unresolved fault never recomputes to `ship`.
