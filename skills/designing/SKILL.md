...
- An open identity offers the preview once, and renders nothing before the answer. The offer is its own message with nothing else in it, sent where the direction is the open question: one question through the harness's structured question tool where it has one, otherwise two numbered lines that end the turn, in the conversation's own language. It names the decision, which direction the build takes, and gives two options, the preview first: **show the directions in a browser preview**, whose description says the <n> directions open side by side as live pages in a browser tab, that building them costs about 3,500 extra tokens per direction and one to three minutes, and that one click picks; and **decide in text**, whose description names the recommended contract's `title` and says it costs nothing extra. An offer that drops the price is not the offer, because a chooser who was not told the price did not agree to it. The preview option writes every comp and starts `pick.mjs`, which opens the tab once the last comp exists; the text option, or an exit 3, makes the `--recommend` contract the selection, with no further question to the terminal. Nothing else is asked through the browser: scope, content, data and behavior are terminal questions, because a question about a visual topic is not a visual question.
...
- A planning turn makes the same offer, because a direction frozen without it was chosen for the user. On the preview option it runs the preview under `$RUN`, which writes nothing in the repository, and freezes the click with `--select --index <n>`; on the text option, or inside a read-only planning mode that blocks writes under `$RUN`, it writes the space file, deals `--plan --seed <token> --space <file> --variants 2`, keeps and fills one contract, `--check`s it, and freezes it with `--select --index 0`. Either way that output is carried verbatim in the Edit block of the plan's first Build step, which writes docs/design/direction.json.
...
Decide the direction in this session, before any production code changes, under `references/phase-direction.md`: the contract, variants, offer, picker and freeze for an open identity, and the single direction every other case produces.
