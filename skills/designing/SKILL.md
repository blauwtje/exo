...
- Rungs 3 and 7 of `## Route` offer the preview once, and render nothing before the answer. The offer is its own message with nothing else in it, sent where the direction is the open question. It names the decision, which direction the build takes, and the price in plain words: the <n> directions open side by side as live pages in a browser tab, building them costs about 3,500 extra tokens per direction and one to three minutes, one click picks, and deciding in text costs nothing extra. Two options in the shape `## A question` in `using-exo` gives end the message, and the user answers with the digit:

  ```text
  1. **Browser preview (Recommended)**: open the <n> directions side by side
  2. **Decide in text**: build <title of the recommended contract>
  ```

  An offer that drops the price is not the offer, because a chooser who was not told the price did not agree to it. The preview option writes every comp and starts `pick.mjs`, which opens the tab once the last comp exists; the text option, or an exit 3, makes the `--recommend` contract the selection, with no further question to the terminal. Nothing else is asked through the browser: scope, content, data and behavior are terminal questions, because a question about a visual topic is not a visual question.
