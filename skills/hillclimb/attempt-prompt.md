...
The text `hillclimb` hands a `general-purpose` delegate on `sonnet` to build one attempt in its own worktree, so the session reviews a diff and measures it rather than typing the change.
...
Commit the change as one commit, staging each edited file by name, with the subject `perf(hillclimb): attempt <n>, <hypothesis>`, even when the checks fail.
