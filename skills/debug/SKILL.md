...
2. **Instrument.** Keep two hypotheses not contradicted by the reproduction. Add one observation that produces different outputs for them at their first divergent boundary. Delegate locating that boundary's file and its callers to a `general-purpose` delegate from `../research/scout-prompt.md` and read here only the ranges it names; a search run here stays in context for every later step.
