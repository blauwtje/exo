...
The text `debug` hands a `general-purpose` delegate on `sonnet` for phase (c), the fix phase, once the handoff names a proven cause. The delegate runs Steps 4-6 in its own context and appends the fix part of the handoff.

```text
Fix the proven cause in the handoff below, repository <root>. Load the `exo:debug` skill first and run only Steps 4 to 6 of its loop: predict, fix, prove. Read only the handoff file and the ranges its `Ranges` line names; nothing else in the repository is in scope.
