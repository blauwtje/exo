---
name: visual-parity
description: Use when the user invokes it to prove that a refactor, migration or dependency bump leaves every rendered surface pixel-identical. Not for a change meant to look different, which designing owns, or a behavior pin with no rendered surface, which refactoring owns.
...
- Not for a change that aims at a new look: `designing` owns that, and its critique judges change this skill forbids.
- Not for a behavior-preserving change with no rendered surface: `refactoring` pins that with a test.
