...
`debug` owns an unproven failure until its cause is established. The frontend-design skill the executing session has loaded owns visual decisions during Build; this skill retains orientation, ordering, non-visual wiring, proof, critique, and reporting. A `general-purpose` delegate on `sonnet` from `../research/scout-prompt.md` owns read-only discovery; `code-review` owns the review context.
