...
8. **Report.** Close under the closing rule in `using-exo`: the goal reached, the proof command with its result, and the one open action. The ledger's final states, a decision made on the user's behalf as one line, and any blocked edit belong there; the reasoning behind a decision and the work outside the request do not.
