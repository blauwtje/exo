...
8. **Report.** Close under the closing rule in `using-exo`: the goal reached, the proof command with its result, and the one open action. A decision made on the user's behalf as one line and any blocked edit belong there, while the list already carries the ledger's final states; the reasoning behind a decision and the work outside the request do not.
