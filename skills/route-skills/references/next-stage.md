...
1. **Two options.** The options follow the question shape: the next stage and Stop. After `define-scope`: 1. Planning, 2. Stop. After `draft-plan`: 1. Implementing, 2. Stop. Picking a stage runs its command, such as `/exo:draft-plan <spec>`, in this session.
2. **Continuing is recommended**, as `1. **<Stage> (Recommended)**`, because this session holds the facts the next stage needs and a clear pays off only once the context is large. Stop is number 2, and its text names the command to run after a context clear, such as `/exo:run-plan <plan path>`.
3. **After a context notice, stopping is recommended.** Once an `exo: context` notice fired this session, Stop moves to `1. **Stop (Recommended)**` and the stage to number 2, because the brief, plan and branch are on disk, so a clear loses nothing the next stage reads. `scripts/next-stage.mjs` reads that notice from the session record, so its printed order is the one shown.
