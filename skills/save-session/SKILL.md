...
1. Read the location, never guess it: `node skills/save-session/scripts/handoff.mjs path` prints it, the same file `hooks/session-start.sh` points a resuming session at. Nothing under that path is committed, and a linked worktree has one of its own, so a handoff cannot follow the wrong branch.
