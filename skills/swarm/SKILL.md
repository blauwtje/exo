---
name: swarm
description: Use when the user invokes it to fan one job out to parallel workers for split coverage, a race, a gauntlet of checks, or an arena of rubric-judged candidates. Not for a plan's tasks, which implementing runs in waves, or one lookup, which exo:explorer answers.
...
- Not for a plan's tasks: `implementing` fans those out in waves.
- Not for one read-only lookup: the `exo:explorer` agent answers it.

## The run

Each delegate brief follows the delegation contract of the user's `CLAUDE.md` and the reader budget in using-exo; this skill adds only what N workers need.
...
2. **Fix the judge.** Also in `frame.md`: a race names `first pass`, `rank all` or `best-of`; a gauntlet passes only when every check passes; an arena gets 3 to 6 gradeable criteria for what success means for this artifact. The rubric never enters a candidate brief, because a candidate that sees it writes to the grader.
3. **Disjoint scopes.** No two workers share a slice or a writable path: code goes to a worktree each, made as `../implementing/references/wave-worktrees.md` makes one, and prose to a file each. A brief that measures names the exact SHAs and the method (sample count, one sample, order), or two numbers cannot be compared.
...
5. **Aggregate.** A result missing its named SHAs or method reruns once; a second miss is a gap, and a gap never counts as a pass. Coverage needs a result for every slice; race and gauntlet apply the rule from `frame.md`. Coverage, race and gauntlet end here at step 8.
...
- A rule in `frame.md` outranks what the results suggest: a rule that looks wrong afterwards means a reframe and a rerun, never a quiet swap.
