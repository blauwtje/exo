...
- `/exo:settings` with nothing after it, or a request to set up or configure exo as a whole: take `## The walk`.
- A question about one setting, or a request to change one: take `## One setting`.
- Not for the savings figures, which `savings` reports.
