---
name: bug-fixer
description: "Use only when the next skill names it, for a checkpoint verification or review finding that fails on a Sonnet turn without evidence naming the causal line: it runs the debug skill on Opus in its own context and returns the proven mechanism, the edits it made, and the proof output. Give it the symptom, the failing command with its output or log path, and the paths in scope. A bug the user reports, a failure in an Opus or Fable session, and a diagnostic that already names the exact file, line, and invalid symbol all belong to the debug skill in the session, not here."
tools:
  - Read
  - Grep
  - Glob
