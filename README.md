...
The segment reads `saved ≈ 1.2k LOC · 340k tok · $12.10 · 1h05`, and a loss prints with a minus. The full panel, markdown the model relays unfenced so Claude Code styles it, with the switch state, the total saved in all projects above today's gain, the run of consecutive days with a saving and a bar to the next dollar milestone (1, 2, 5, 10, 20, ...), then the saving in the current project beside all projects, and a 30-day trend of the cost saved per day for each: `/exo:savings`, or `node "$(cat ~/.claude/exo/plugin-root)/skills/savings/scripts/savings.mjs" report`.
