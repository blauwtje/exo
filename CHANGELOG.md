...
- `implementing`'s `next-task.mjs` writes each wave task's frame and section to a brief file under the run checkout's git directory and prints only its `Design:`, `Run:`, drift and `Brief:` lines, so the dispatch to `exo:implementer` names that path instead of pasting the task twice into the session; the brief drops `Conventions:` and the implementer no longer rereads the root `CLAUDE.md` it already loaded.
