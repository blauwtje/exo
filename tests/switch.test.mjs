...
test('the session hook leads with the settings line resolved for the project', { skip: withoutJq }, async () => {
  ...
  assert.equal(result.code, 0, result.stderr);
  const context = JSON.parse(result.stdout).hookSpecificOutput.additionalContext;
  assert.ok(context.startsWith('exo settings: specs=issues (project), replies=tight (default), context=100 (default), budget=normal (default), ship=ask (default), workspace=ask (default). Replies are tight:'), context.slice(0, 200));
});
