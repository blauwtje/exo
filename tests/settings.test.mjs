...
test('with nothing set the schema default applies', async () => {
  ...
  assert.equal(result.stdout.trim(), 'exo settings: specs=docs (default), replies=tight (default), context=100 (default)' + TIGHT_RULE);
});
...
test('local outranks project, which outranks global', async () => {
  ...
  assert.equal((await settings(shared, ['context'])).stdout.trim(), 'exo settings: specs=issues (project), replies=tight (default), context=100 (default)' + TIGHT_RULE);
  const globalOnly = await workspace({ global: { specs: 'both' } });
  assert.equal((await settings(globalOnly, ['context'])).stdout.trim(), 'exo settings: specs=both (global), replies=tight (default), context=100 (default)' + TIGHT_RULE);
});
...
test('the hook environment carries the global value when it is set', async () => {
  ...
  assert.equal(result.stdout.trim(), 'exo settings: specs=issues (global), replies=tight (default), context=100 (default)' + TIGHT_RULE);
});
...
test('a project file that is not JSON is named in the context line, and defaults apply', async () => {
  ...
  assert.match(result.stdout, /^exo settings: specs=docs \(default\), replies=tight \(default\), context=100 \(default\); .*exo\.json is not valid JSON/);
});
