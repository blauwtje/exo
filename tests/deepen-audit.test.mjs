// The deepen audit dispatches a read-only delegate that writes ranked cards
...
const PROMPT = path.join(ROOT, 'skills/deepen/auditor-prompt.md');
const SKILL = path.join(ROOT, 'skills/deepen/SKILL.md');
...
test('SKILL.md dispatches the delegate instead of reading scoped code itself', () => {
  ...
  assert.ok(!text.includes('commit history keeps coming back'), 'SKILL.md still has the explorer computing churn');
});
