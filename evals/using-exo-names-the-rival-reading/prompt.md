...
The user asked, in repository `ledger-panel`: "Show savings per project." Nobody clarified it. What happened this turn:

- `src/panel/render.mjs` now renders one column per project, and `src/panel/columns.mjs` builds those columns from the ledger's existing project key.
