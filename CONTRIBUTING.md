...
`hooks/hooks.json` wires nine hook groups:
...
- **PreToolUse** on every tool: the delegate budget, in `skills/savings/scripts/delegate-budget.mjs`. Only inside a delegate, which carries `agent_id`, it reads the tail of `<transcript without .jsonl>/subagents/agent-<agent_id>.jsonl` for the last assistant turn's input, cache read and cache creation tokens, and counts the call in `<tmpdir>/exo-delegate-budget/<agent_id>`. Past the soft limit it adds one `exo budget: <n>k of <hard>k tokens used` line; past the hard limit, in tokens or calls, it denies every tool but `Edit`, `Write`, `TaskUpdate` and `TodoWrite`, with a reason that says to write the report and list what is open under `Unresolved`. Limits default to 40k, 70k and 60 calls in `skills/savings/assets/delegate-budgets.json`, whose `agents` map overrides them per `agent_type`; a `Budget: <soft>k/<hard>k` line in the dispatch, optionally ending `/<calls> calls`, outranks both. The main session, a missing transcript, one without a complete usage line and any fault print nothing.
