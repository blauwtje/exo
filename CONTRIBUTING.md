...
- Nothing is multiplied by a benchmark ratio and nothing is derived from a character count. The report prints what exo cost and what it held back side by side and never a net: cost is in tokens and held-back text in bytes, and no measured rate turns one into the other. `benchmarks/` measures what the same work costs without exo, separately.
