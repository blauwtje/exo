...
A plan is asked for, a read-only planning mode is active, another session or executor will run the work, or the work holds two or more edit-order dependencies. A shaped issue goes straight to planning and is never shaped again.
...
- A question offering implementing or implementing-batch, with the model each wants.

## Where its rules live

`skills/planning/SKILL.md`, with the artifact shape in its `references/plan-spec.md`.
