...
`skills/using-exo/SKILL.md`. `hooks/session-start.sh` injects its body, and `verify/checks/injected-context.mjs` locks the size of what it injects, because that text is the most expensive thing exo owns.
