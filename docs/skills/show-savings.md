...
- A switch line under the report pointing at `/exo:configure counter off`, where the counter and the read guard are switched.

## Where its rules live

`skills/show-savings/SKILL.md`, with the record and guard scripts beside it. The size of one diff is git's job, and the live figure is the status line segment's.
